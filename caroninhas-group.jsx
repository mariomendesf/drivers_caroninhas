...
const TRIP_CUTOFF_HOUR = 19; // trips are considered "closed" for the day only at/after this hour
...
// Latest date (YYYY-MM-DD, local) a non-admin driver may register their own trip for.
// Before TRIP_CUTOFF_HOUR, today's trip hasn't "closed" yet, so the limit is yesterday.
function maxRegistrableDate(now = new Date()) {
  const cutoff = new Date(now);
  cutoff.setHours(TRIP_CUTOFF_HOUR, 0, 0, 0);
  const limit = new Date(now);
  if (now < cutoff) limit.setDate(limit.getDate() - 1);
  return localDateStr(limit);
}

// A pending trip can only be confirmed by its driver from TRIP_CUTOFF_HOUR on the trip's own date.
function canConfirmTripNow(dateStr, now = new Date()) {
  const cutoff = new Date(`${dateStr}T${String(TRIP_CUTOFF_HOUR).padStart(2, "0")}:00:00`);
  return now >= cutoff;
}
...
function Home({ st, upd, setTab, myId }) {
  ...
  const tryStartEditPending = (tr) => {
    if (myId !== ADMIN_ID && !canConfirmTripNow(tr.date)) {
      setConfirmToast(t("confirm_too_early"));
      return;
    }
    startEditPending(tr);
  };
  ...
  const confirmTrip = (tripId) => {
    const target = st.trips.find(tr => tr.id === tripId);
    if (target && myId !== ADMIN_ID && !canConfirmTripNow(target.date)) {
      setConfirmToast(t("confirm_too_early"));
      return;
    }
  ...
  return (
    <div>
        ...
        <div style={{ padding: "52px 20px 0" }}>
          <div style={{ background: C.amberDim, border: `1px solid ${C.amber}44`, borderRadius: 14, padding: "14px 16px", marginBottom: 4 }}>
            ...
            {pendingTrips.map(tr => {
              ...
              const pendingDriver = st.drivers.find(d => d.id === tr.driverOwnerId);
              ...
              return (
                <div key={tr.id} style={{ background: C.card, borderRadius: 10, padding: "12px 14px", marginBottom: 8 }}>
                  ...
                  <div style={{ fontSize: 12, color: C.amber, fontWeight: 700, marginBottom: 2 }}>
                    {t("driver_label")}: {pendingDriver?.name || tr.driverOwnerId}
                  </div>
                  ...
                  {!isEditing ? (
                    <>
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={() => tryStartEditPending(tr)} style={{ flex: 2, background: C.amberDim, border: `1px solid ${C.amber}44`, color: C.amber, borderRadius: 9, padding: "9px 0", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "Barlow, sans-serif" }}>
                          {t("edit_confirm_btn")}
                        </button>
      ...
      {confirmToast && <Toast msg={confirmToast} onDone={() => setConfirmToast(null)} />}
    </div>
  );
}
...
function AddTrip({ st, upd, setTab, myId }) {
  const lang = st?.lang || "pt";
  const t = mkT(lang);
  const [date, setDate] = useState(() => myId === ADMIN_ID ? todayStr() : maxRegistrableDate());
  ...
  const save = () => {
    ...
    // once it's actually over (past TRIP_CUTOFF_HOUR).
    const isOnBehalfOfOther = (role === "passageiro" && !isCustomDriver && driverId) || (role === "motorista" && !!overrideDriver);
    if (!isOnBehalfOfOther && myId !== ADMIN_ID && date > maxRegistrableDate()) {
      setToast(t("add_future_blocked"));
      return;
    }
    // Build trip object
    let trip;
      ...
      <div style={{ padding: "0 20px", display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div style={cardS}>
          ...
          <input type="date" value={date} onChange={e => setDate(e.target.value)} style={inp}
            max={(((role === "passageiro" && !isCustomDriver && driverId) || (role === "motorista" && !!overrideDriver)) || myId === ADMIN_ID) ? undefined : maxRegistrableDate()} />
        </div>
...
function Saldos({ st, upd, myId, initialView }) {
  ...
  // Passageiro trips with a non-group (custom) driver — driverId absent, driverName present
  const avulsoDebtTrips = trips.filter(t =>
    !t.pendingConfirmation &&
    t.role === "passageiro" &&
    t.registeredBy === myId &&
    !t.driverId &&
  ...
  const avulsoIOwePending = avulsoDebtTrips
    .filter(t => !t.paid && !t.settledByRide)
    .reduce((s, t) => s + tripPrice(t) * (t.direction === "ambas" ? 2 : 1), 0);

  const totalIOwePending    = Object.values(driverNet).reduce((s, dn) => s + Math.max(0, dn.iOwe - dn.iOwePaid), 0) + avulsoIOwePending;
    ...
    <div>
      ...
      {view === "saldos" && (
        <div style={{ padding: "0 20px", display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {avulsoDebtTrips.some(t => !t.paid && !t.settledByRide) && (
            <div>
              <SectionLabel>{lang === "pt" ? "Dívidas com motoristas avulsos" : "Non-group driver debts"}</SectionLabel>
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {avulsoDebtTrips.filter(t => !t.paid && !t.settledByRide).map(tr => {
                  const x = tr.direction === "ambas" ? 2 : 1;
                  const amt = tripPrice(tr) * x;
                  return (
                    <div key={tr.id} style={{ ...cardS, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div>
                        <div style={{ fontSize: 14, color: C.text, fontWeight: 600 }}>{tr.driverName}</div>
                        <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>
                          <span style={{ color: C.amber, fontWeight: 600, marginRight: 4 }}>{weekday(tr.date, lang)}</span>
                          {fmtFull(tr.date)} · {DIR(lang)[tr.direction]}
                        </div>
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 18, fontWeight: 700, color: C.red }}>−{R(amt)}</span>
                        <button onClick={() => markPaid(tr.id)} style={btnGreen}>{t("extrato_pay")}</button>
                      </div>
                    </div>
                  );
                })}
