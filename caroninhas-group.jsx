    ...
    try {
      ...
      if (snap.exists) {
        ...
        if (!fsData || !Array.isArray(fsData.trips)) {
          ...
          const lsData = lsGet();
          const useData = (lsData?.trips?.length > fsData.trips.length) ? lsData : fsData;
          if (useData === lsData) {
            console.log("[grupo] localStorage newer, syncing to Firestore...");
            await db.collection("caronas").doc(KEY).set({ payload: stripUndefined(lsData) });
          }
          console.log("[grupo] Loaded OK, trips:", useData.trips.length);
          lsSet(useData);
          return useData;
        }
      }
        ...
        <div style={editBar}>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <div style={{ marginBottom: 2 }}>
              {!readOnly && trip.registeredBy === myId && !editingDate && (
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <span style={{ fontSize: 12, color: C.muted }}>📅 {fmtFull(trip.date)}</span>
                  <button onClick={() => { setDateVal(trip.date); setEditingDate(true); }} style={{ background: "none", border: "none", color: C.muted, fontSize: 11, cursor: "pointer", padding: 0, fontFamily: "Barlow, sans-serif" }}>{t("trip_edit_date")}</button>
                </div>
              ...
              {(readOnly || trip.registeredBy !== myId) && (
                <span style={{ fontSize: 12, color: C.muted }}>📅 {fmtFull(trip.date)}</span>
              )}
            </div>
            {pax.map(p => {
              ...
              const iAmDriver = !readOnly && trip.registeredBy === myId;
              return (
          ...
          {!readOnly && trip.registeredBy === myId && <CarCostEditor trip={trip} st={st} upd={upd} />}
          {!readOnly && trip.registeredBy === myId && <div style={{ fontSize: 11, color: C.muted, marginTop: 10 }}>{lang === "en" ? "Tap any status again to undo." : "Toque novamente em qualquer status para desfazer."}</div>}
        </div>
      )}
    </div>
  );
    ...
    <div>
      ...
      <div style={{ padding: "0 20px", display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <StatCard label={t("home_owe")} value={R(totalOwed)} color={totalOwed > 0 ? C.red : C.green}
            sub={totalOwed > 0 ? t("home_to_drivers") : quitadoComCarona ? t("home_cancelled_ride") : t("home_all_paid")} />
          <StatCard label={t("home_to_receive")} value={R(totalPending)} color={totalPending > 0 ? C.amber : C.green}
            sub={totalPending > 0 ? t("home_from_pax") : quitadoComCarona ? t("home_cancelled_ride") : t("home_all_received")} />
        </div>
                                  ...
                                  return (
                                    <div key={p.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0", borderBottom: `1px solid ${C.dim}` }}>
                                      ...
                                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                        ...
                                        {!p.paid && !settled && <button onClick={() => markPaxPaid(trip.id, p.id)} style={btnGreen}>{t("trip_received")}</button>}
                                      </div>
                                    </div>
                                  );
                                });
                            ...
                            return (
                              <div key={p.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                ...
                                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                  ...
                                  {p.paid
                                    ? <span style={{ color: C.green, fontSize: 12 }}>{t("received_label_short")}</span>
                                    : <button onClick={() => markPaxPaid(trip.id, p.id)} style={btnGreen}>{t("trip_received")}</button>}
                                </div>
                              </div>
                            );
                          })}
...
function Opcoes({ st, upd, myId, onSignOut }) {
  ...
  const saveCarCosts = () => {
    const dcc = { ...(st.driverCarCosts || {}), [myId]: { gas: parseFloat(gasVal) || DEF_GAS, toll: parseFloat(tollVal) || DEF_TOLL } };
